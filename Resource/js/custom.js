"use strict";var windowElement=jQuery(window);var body=jQuery("body");var current_track=1;var current_slide=0;var slides_length=0;var prev_slide=0;var slideWidth=$("#popup-container").width()*0.8;var appendBody;var popup_images_arr=[];function getDuration(src){return new Promise(function(resolve){var audio=new Audio();$(audio).on("loadedmetadata",function(){resolve(audio.duration);});audio.src=src;});}
...
e.preventDefault();}});function setTeamMemberHeight(){$('.music-banner').height($('.music-banner').width());$('.music-banner-image').height($('.music-banner-image').width());$('.news_bg_image').height($('.news_bg_image').width());$('.disc-image').height($('.music-banner-image').width()-30);$('.disc-image').width($('.disc-image').height());$('.s-item-image').height($('.s-item-image').width());}
...
setTeamMemberHeight();window.msnry=new Masonry('.grid',{columnWidth:'.grid-sizer',percentPosition:true});$('.more').on("click",function(){var html='<div class="grid-item grid-item--width1" >'+
...
$(".overly-1").animate({opacity:0},300,function(){$(this).css("display","none");});$("#text-and-video").css('position','relative');});$("body.single-events").on("click",function(event){event.stopPropagation();$(".overly-1").css("display","block").animate({opacity:1},300);$("#text-and-video").css('position','initial');});$(document).ready(msieversion);if(navigator.userAgent.search("NET")>=0){if(!Modernizr.objectfit){$('.music-banner-image').each(function(){var $container=$(this),imgUrl=$container.find('img').prop('src');if(imgUrl){$container.css('backgroundImage','url('+imgUrl+')').addClass('compat-object-fit');}});$('.embed-responsive .youtube').each(function(){var $container=$(this),imgUrl=$container.find('img').prop('src');if(imgUrl){$container.css('backgroundImage','url('+imgUrl+')').addClass('compat-object-fit');}});$('.video-block .video-img-banner').each(function(){var $container=$(this),imgUrl=$container.find('img').prop('src');if(imgUrl){$container.css('backgroundImage','url('+imgUrl+')').addClass('compat-object-fit');}});$('.grid-item-img').each(function(){var $container=$(this),imgUrl=$container.find('img').prop('src');if(imgUrl){$container.css('backgroundImage','url('+imgUrl+')').addClass('compat-object-fit');}});$('.blog-grid-image').each(function(){var $container=$(this),imgUrl=$container.find('img').prop('src');if(imgUrl){$container.css('backgroundImage','url('+imgUrl+')').addClass('compat-object-fit');}});$('.featured-image').each(function(){var $container=$(this),imgUrl=$container.find('img').prop('src');if(imgUrl){$container.css('backgroundImage','url('+imgUrl+')').addClass('compat-object-fit');}});$('.t-shirt-image').each(function(){var $container=$(this),imgUrl=$container.find('img').prop('src');if(imgUrl){$container.css('backgroundImage','url('+imgUrl+')').addClass('compat-object-fit');}});}}
